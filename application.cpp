   ...
   vec3 Get_Particle_Color(float d) {
   	if(d < 0.1){ 
		return vec3(255,255,0); 
		 ...
		 return vec3(255,255*(1.5-d),0);
	}
    	else if(d < 2){
		return vec3(255,0,0); 
	}
    	else if(d < 3){
		return vec3(255*(3-d),0.5,0.5); 
	}
    	else{ 
...
void application::draw_event()
{
	...
	for(unsigned int i = 0; i < PList.size(); i++){
	    PList.at(i).f += vec3(0, -PList.at(i).m * 9.8, 0);
	    PList.at(i).Euler_Step(h);
	    PList.at(i).Handle_Collision(0.5,0.5);
	    PList.at(i).Reset_Forces();
    	    PList.at(i).d += h;
   	    PList.at(i).color = Get_Particle_Color(PList.at(i).d);
	}
   }
   ...
   for (unsigned int i = 1; i < PList.size(); ++i) {
	vec3 color = PList.at(i).color;
	vec3 start = PList.at(i).x;
	vec3 end = start + 0.04f*PList.at(i).v;
